...
// INCLUSIÓN DE LOS NUEVOS ARCHIVOS MODULARES
require_once plugin_dir_path( __FILE__ ) . 'includes/map-categories.php';
require_once plugin_dir_path( __FILE__ ) . 'includes/map-layout.php';
...
$args = array(
        ...
        'posts_per_page' => -1,
        'post_status'    => 'publish',
        'orderby'        => 'title', // Le decimos que ordene por el título
        'order'          => 'ASC'    // Le decimos que sea de la A a la Z
    );
    ...
    // LLAMADA A LA FUNCIÓN QUE TRAE EL HTML Y CSS DE LA ESTRUCTURA
    $html = mapa_pai_obtener_layout_directorio( $dimensiones, $color_sidebar );
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // LLAMADA A LA FUNCIÓN QUE TRAE EL JS DE LAS CATEGORÍAS
        ' . mapa_pai_obtener_js_categorias( $color_sidebar ) . '

        function procesarFiltros() {
            var terminoBusqueda = inputBuscador.value.toLowerCase().trim();
            var categoriasActivas = Array.from(document.querySelectorAll(".filtro-cat:checked")).map(c => c.value);

            todosLosMarcadores.forEach(m => {
                var coincideCategoria = m.categorias.some(c => categoriasActivas.includes(c));
                var coincideBusqueda = terminoBusqueda === "" || m.textoBusqueda.includes(terminoBusqueda);
