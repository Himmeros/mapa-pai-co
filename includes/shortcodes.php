...
function mapa_pai_shortcode_directorio() {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        controlCategorias.onAdd = function (map) {
            ...
            div.innerHTML = "<h4 style=\'margin-top:0; color:#fff;\'>Categorías</h4><div class=\"contenedor-filtros-scroll\">" + 
                Array.from(categoriasUnicas).sort().map(cat => `<label style=\'display:block; margin-bottom:5px;\'><input type="checkbox" class="filtro-cat" value="${cat}" checked> ${cat}</label>`).join("") + 
                "</div>";
